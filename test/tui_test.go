...
func cockpit(t *testing.T, dir, pod string) *ptyRun {
	t.Helper()
	r := onPTYIn(t, dir, "cockpit", pod)
	if !r.waitFor(t, "MACHINES", 20*time.Second) {
...
func TestCockpitMountsWithOneKey(t *testing.T) {
	...
	defer r.stop()
	r.send("m")
	if !r.waitFor(t, "mount (host:/path)", 5*time.Second) {
		t.Fatalf("`m` did not prompt; got:\n%s", r.out.String())
	}
	r.send("vptest2:" + remoteAlt + "\r")
	if !r.waitFor(t, "done", 25*time.Second) {
...
func TestCockpitMovesASessionsBackend(t *testing.T) {
	...
	sh := onPTYIn(t, dir, "shell", "e2e-tui-backend")
	defer sh.stop()
	...
	defer r.stop()
	if !r.waitFor(t, "▸ pod", 10*time.Second) {
		t.Fatalf("the cockpit does not show the session's machine; got:\n%s",
			r.out.String())
	}
	r.send("\t") // focus the sessions
	r.send("b")
	if !r.waitFor(t, "backend for this session", 5*time.Second) {
		t.Fatalf("`b` did not prompt; got:\n%s", r.out.String())
	}
	r.send("vptest\r")
	if !r.waitFor(t, "now on vptest", 20*time.Second) {
...
func TestCockpitHandsTheTerminalToASessionAndTakesItBack(t *testing.T) {
	...
	// exiting it.
	sh := onPTYIn(t, dir, "shell", "e2e-tui-handoff")
	sh.ready(t, 15*time.Second)
	...
	r.send("\x1c")
	if !r.waitFor(t, "left running", 15*time.Second) {
